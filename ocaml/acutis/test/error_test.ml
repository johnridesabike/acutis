...
let matching_unused () =
  ...
  check_raises "Nest patterns merge into wildcard patterns correctly (1)."
    (Error "File <test>, 1:31-1:46\nMatching error.\nThis match case is unused.")
    (render "{% match a, b with x, y %} {% with (_, _), 40 %} {% /match %}");
  check_raises "Nest patterns merge into wildcard patterns correctly (1)."
    (Error "File <test>, 4:4-4:22\nMatching error.\nThis match case is unused.")
    (render
       "{% match a, b\n\
       \   with x, 1 %}\n\
        {% with (\"a\", \"b\"), 10 %}\n\
