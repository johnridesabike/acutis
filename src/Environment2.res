...
type t<'a> = Optimize.environment<'a>
...
let rec makeEnv = (. stack): t<_> => {
  renderRoot: (. {name, nodes, propTypes}, props, children) =>
    try {
      reduceQueue(.
        Runtime.make(
          ~nodes,
          ~props=Props.make(propTypes, props),
          ~children,
  ...
  render: (. {name, nodes, propTypes: _}, props, children) =>
    reduceQueue(.
      Runtime.make(
        ~nodes,
        ~props,
