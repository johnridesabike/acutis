...
module Pattern = {
  ...
  let rec parseNode = tokens => {
    ...
    | Identifier(loc, Id("null")) => Null(loc)
    | Identifier(loc, Id("false")) => False(loc)
    | Identifier(loc, Id("true")) => True(loc)
    | Identifier(loc, Id(x)) => Binding(loc, Id(x))
  ...
  and parseObjectKeyValue = tokens => {
    switch Lexer.popExn(tokens) {
    | JsonString(loc, key) | Identifier(loc, Id(key)) =>
      switch Lexer.peekExn(tokens) {
      | Colon(_) =>
        Lexer.skipExn(tokens)
      ...
      | _ =>
        let value = Id(key)
        if RegEx.isLegalBinding(value) {
          (key, Binding(loc, value))
        } else {
          raise(
            IllegalIdentifier({
              loc: loc,
  ...
  | Identifier(loc, head) when RegEx.isLegalBinding(head) =>
    let rec aux = l =>
      ...
      | Comma(_) =>
        ...
        | Identifier(loc, x) =>
          raise(IllegalBindingName({loc: loc, binding: x, name: Lexer.name(tokens)}))
        | x =>
          raise(
            UnexpectedToken({
              token: x,
...
let endOfMatchMap = (. x: Tokens.t) =>
  switch x {
  | Identifier(_, Id("with")) | Slash(_) => true
  | _ => false
  }
  ...
  let rec aux = l =>
    if until(. Lexer.peekExn(tokens)) {
      ...
      | Tilde(_) =>
        ...
        | x => raise(UnexpectedToken({token: x, name: Lexer.name(tokens)}))
        }
      | Comment(_) => aux(l)
      | Identifier(loc, Id("match")) =>
      ...
      | EchoIdentifier(loc, x) =>
        raise(IllegalBindingName({loc: loc, binding: x, name: Lexer.name(tokens)}))
      | Identifier(loc, x) =>
        raise(InvalidStatement({statement: x, loc: loc, name: Lexer.name(tokens)}))
      | x =>
        raise(
          UnexpectedToken({
            token: x,
...
and parseCaseBlocks = (tokens, ~block) => {
  let rec aux = l =>
    ...
    | Slash(_) =>
      Lexer.skipExn(tokens)
      switch Lexer.popExn(tokens) {
      | Identifier(_, Id(x)) when x == block => List.reverse(l)
      | x =>
        raise(
          UnexpectedToken({
            token: x,
...
and parseComponent = (loc, name, tokens) => {
  ...
  | _ =>
    ...
    Lexer.skipExn(tokens)
    let children = list{(Id("Children"), ChildBlock(x)), ...children}
    switch Lexer.popExn(tokens) {
...
let makeAstJs = (. source, name) =>
  switch Js.typeof(source) {
  | "string" => makeAst(source, ~name?)
  | x => raise(BadInputTemplate(x))
  }
...
let makeJs = (. source, name) =>
  switch Js.typeof(source) {
  | "string" => make(source, ~name?)
  | x => raise(BadInputTemplate(x))
  }
