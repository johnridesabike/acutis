...
let pp_statements ty ppf l = Seq.iter (pp_statement ty ppf) l
...
type component = { name : string; nodes : C.jsfun C.template C.nodes }
...
let make_js_imports components =
  Map.String.fold
    (fun name v acc ->
      ...
      | C.Fun (typescheme, Compile.{ module_path; function_path }) ->
...
let pp ty ppf x =
  ...
  Id.Map.to_seq imports
  |> Seq.map (fun (namespace, Filepath filename) ->
         Import (namespace, filename))
  |> pp_statements ty ppf;
  List.to_seq Err.defs
  |> Seq.map (fun (id, str) -> Let (id, str))
  |> pp_statements ty ppf;
  pp_statement ty ppf (Raw raw_functions);
  List.to_seq components_imports
  |> Seq.map (fun { name; typescheme; namespace; function_path } ->
  ...
  |> pp_statements ty ppf;
  List.to_seq components
  |> Seq.map (fun { name; nodes } ->
         Fun
           ( Id.component name,
